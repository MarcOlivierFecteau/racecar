...
class SlashController(Node):
    def __init__(self):
        ...
        # Init subscribers
        self.sub_ref: Subscription = self.create_subscription(
            Twist, "ctl_ref", self.read_ref, 1
        )
        self.sub_prop: Subscription = self.create_subscription(
            Float32MultiArray, "prop_sensors", self.read_arduino, 1
        )
        self.sub_laser: Subscription = self.create_subscription(
