...
class SlashController(Node):
    def __init__(self):
        ...
        # Init publishers
        self.pub_cmd: Publisher = self.create_publisher(Twist, 'prop_cmd', 1)

        # Timer
        self.dt = 0.05
        self.timer: Timer = self.create_timer(self.dt, self.timed_controller)
        ...
        self.K_autopilot =  None # TODO: DESIGN LQR (APP4)
    
        self.K_parking   =  None # TODO: DESIGN PLACEMENT DE POLES (APP4)
    ...
    def timed_controller(self) -> None:
        ...
        if(self.high_level_mode == -1):   # Full stop mode
            self.propulsion_cmd = 0 # Command sent to propulsion
            self.arduino_mode = 0   # Control mode
            self.steering_cmd = 0   # Command sent to the steering servo
        else:
            # APP2 (open-loop steering) Controllers Bellow          
            if(self.high_level_mode == 1):  # Open-Loop
            ...
            elif(self.high_level_mode == 0):    # Closed-loop velocity on arduino
                self.propulsion_cmd = self.propulsion_ref
                self.arduino_mode = 2
                self.steering_cmd = self.steering_ref + self.steering_offset
                
            elif(self.high_level_mode == 2):    # Closed-loop position on arduino
                self.propulsion_cmd = self.propulsion_ref
                self.arduino_mode = 3
                self.steering_cmd = self.steering_ref + self.steering_offset
            
            # APP4 (closed-loop steering) controllers bellow
            elif(self.high_level_mode == 3 or self.high_level_mode == 5):   # Closed-loop velocity and steering
